...
Game_object* cur_obj;
string cur_obj_name;
stack<Statement_block*> stmt_block_stack;
vector<Animation_block*> anim_block_vec;
...
%type <union_expression> geometric_operator
...
program:
  ...
  {
    if (!anim_block_vec.empty()) {
    vector<Animation_block*>::iterator it;
    for (it = anim_block_vec.begin(); it < anim_block_vec.end(); it++) {
      if (!((*it)->complete())) {
        Error::error(Error::NO_BODY_PROVIDED_FOR_FORWARD, (*it)->name());
        break;
      }
    }
    }
...
forward_declaration:
  T_FORWARD T_ANIMATION T_ID T_LPAREN animation_parameter T_RPAREN {
    ...
    anim_block_vec.push_back((Animation_block*)(symbol->value));
  }
  ;
...
animation_block:
  T_ANIMATION T_ID {
      ...
      sym = Symbol_table::instance()->get_symbol(*$2);
      if (!sym) Error::error(Error::NO_FORWARD_FOR_ANIMATION_BLOCK, *$2);
      else {
        block = (Animation_block*)(sym->value);
        if (block->complete())
          Error::error(Error::PREVIOUSLY_DEFINED_ANIMATION_BLOCK, block->name());
       }
    } T_LPAREN check_animation_parameter {
      Symbol* sym = NULL;
      Animation_block* block = NULL;
      sym = Symbol_table::instance()->get_symbol(*$2);
      if (sym) block = (Animation_block*)(sym->value); 
      if (block) {
        Game_object* obj = (Game_object*)(block->get_parameter_symbol()->value);
        if (obj->type() != *$5)
          Error::error(Error::ANIMATION_PARAM_DOES_NOT_MATCH_FORWARD);
        block->mark_complete();
        stmt_block_stack.push(block);
      }
...
if_statement:
  T_IF T_LPAREN expression T_RPAREN if_block %prec IF_NO_ELSE
  {
    ...
    stmt_block_stack.top()->statements.push_back(stmt);
  }
...
expression:
  ...
  | expression T_DIVIDE expression
  {
    ...
    if ($1->return_type > DOUBLE) {
      Error::error(Error::INVALID_LEFT_OPERAND_TYPE, "/");
      $$ = new Expression(new int(1), INT, NULL, NULL);
    } else if ($3->return_type > DOUBLE) {
      Error::error(Error::INVALID_RIGHT_OPERAND_TYPE, "/");
      $$ = new Expression(new int(1), INT, NULL, NULL);
    } else if (*((double*)($3->evaluate_to_type(DOUBLE)->value)) == 0) {
  ...
  | expression T_MOD expression
  {
    if ($1->return_type > INT) {
      ...
      Error::error(Error::INVALID_RIGHT_OPERAND_TYPE, "%");
      $3 = new Expression(new int(1), INT, NULL, NULL);
    } else if (*((int*)($3->evaluate_to_type(INT)->value)) == 0) {
  ...
  {
    if ($1->return_type != GAME_OBJECT)
      Error::error(Error::OPERAND_MUST_BE_A_GAME_OBJECT, $1->get_name());
    else if ($3->return_type != GAME_OBJECT)
      Error::error(Error::OPERAND_MUST_BE_A_GAME_OBJECT, $3->get_name());
    $$ = new Expression($2->operator_type,
                        INT,
                        new Expression($1),
                        new Expression($3));
  }
