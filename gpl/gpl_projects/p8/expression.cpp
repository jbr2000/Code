...
Expression* Expression::evaluate_to_type(Gpl_type desired_type) {
    ...
    switch (return_type) {
        ...
        case GAME_OBJECT: return this; break;
        case ANIMATION_BLOCK: assert(0); break;
...
Expression* Expression::evaluate() {
    double const RADIAN_MULTIPLIER = M_PI/180;
    double const DEGREE_MULTIPLIER = 180/M_PI;
    ...
    switch (node_type) {
        ...
        case VARIABLE: return new Expression(this->variable->get_value(),
                                              this->variable->return_type,
                                              NULL, NULL);
            break;
        case NO_CAST:
            return_type = left->return_type;
            break;
        case UNARY_OPERATOR: break;
        case BINARY_OPERATOR: break;
        case GEOMETRIC_OPERATOR: break;
    ...
    switch (return_type) {
        case INT: {
            int left_i_value;
            int right_i_value;
            if (node_type != GEOMETRIC_OPERATOR) {
                left_i_value = *((int*)(left->evaluate_to_type(return_type)->value));
                if (node_type == BINARY_OPERATOR)
                    right_i_value = *((int*)(right->evaluate_to_type(return_type)->value));
                else if (node_type == BINARY_LOGICAL_OPERATOR)
                    right_i_value = *((int*)(right->evaluate_to_type(master_type)->value));
            }
            ...
            switch (operator_type) {
                ...
                // GEOMETRIC_OPERATORS
                case NEAR:    return new Expression(new int(((Game_object*)(left->variable->get_value()))->near(
                                                      ((Game_object*)(right->variable->get_value())))),
                                                        INT, NULL, NULL); break;
                case TOUCHES: return new Expression(new int(((Game_object*)(left->variable->get_value()))->touches(
                                                      ((Game_object*)(right->variable->get_value())))),
                                                        INT, NULL, NULL); break;
                // UNARY
        ...
        case DOUBLE: {
            ...
            left_d_value = *((double*)(left->evaluate_to_type(return_type)->value));
            if (node_type == BINARY_OPERATOR)
                right_d_value = *((double*)(right->evaluate_to_type(return_type)->value));
            else if (node_type == BINARY_LOGICAL_OPERATOR)
                right_d_value = *((double*)(right->evaluate_to_type(master_type)->value));
            ...
            switch (operator_type) {
                ...
                case MOD:      return new Expression(new double(fmod(left_d_value, right_d_value)), DOUBLE, NULL, NULL); break;
                // EQUALITY
        ...
        case STRING: {
            ...
            left_s_value = *((string*)(left->evaluate_to_type(return_type)->value));
            if (node_type == BINARY_OPERATOR)
                right_s_value = *((string*)(right->evaluate_to_type(return_type)->value));
            else if (node_type == BINARY_LOGICAL_OPERATOR)
                right_s_value = *((string*)(right->evaluate_to_type(master_type)->value));
            switch (operator_type) {
